...
FILE_PATH = './24wv_SM_Accumulated_probe_table.csv'
SAVE_PATH = './00_24wv'
N_MODES = 1 # number of eigenvalues sought for in Comsol!!
...
λ0vec = np.linspace(1000,1200,500)*u.nm
...
def new_interpolation(x, y):
    interpolation = interpolate.interp1d(x,y)
    def float_protec(z):
        if np.any(z < np.min(x)):
            print(f"Value outside interpolation range: {np.min(z)} " \
                          f"is less than {np.min(x)}")
            z = np.where(z < np.min(x), np.min(x), z)
        if np.any(z > np.max(x)):
            print(f"Value outside interpolation range: {np.max(z)} " \
                          f"is more than {np.max(x)}")
            z = np.where(z > np.max(x), np.max(x), z)
...
def index_analysis(df, save_path='./'):
    ...
    for i in range(N_MODES):
        #--------------
        neff_list.append(np.real(df['Effective_mode_index (1)'].iloc[i::N_MODES].values)) # take only real part
...
def deltabeta(wavelength, wavelength0, β2_list, lambda_vec, imode):
    ...
    beta2interp = new_interpolation(lambda_vec[:-2], β2) 
...
def gain_analysis(wavelength, wavelength0, β2_list, lambda_vec, γ, P, L, imode=N_MODES, save_path='./'):
    ...
    for g in γ:
        ax0.axhline((-2*g*P).to(1/u.mm).value,c='red')
    ax0.set_ylabel("Δβ [$mm^{-1}$]")
    ax0.grid()
    #------------------
    ax0=ax[1]
    ax0.plot(wavelength.to(u.um),np.real(gvec))
    ax0.set_ylabel("Gain coeff. [$m^{-1}$]")
    ax0.grid()
    #------------------
    ax0=ax[2]
...
def gain_pump_analysis(wavelength, wavelength0, β2_list, lambda_vec, γ, P, L, imode=N_MODES, save_path='./'):
    ...
    #wavelength0 = 1200e-9*u.m # pump wavelength 
        ...
        ax0.plot(wavelength.to(u.um),Δβ)
        for g in γ:
            ax0.axhline((-2*g*P).to(1/u.mm).value,c='black')
        ax0.set_ylabel("Δβ [$mm^{-1}$]")
        ax0.grid()
        #------------------
        ax0=ax[1]
    ...
    save_file = os.path.join(save_path, 'parametric_gain_lp_sweep.png')
    plt.savefig(save_file, bbox_inches='tight', dpi=300)
    plt.clf()
    plt.close()
...
def zero_GVD_wl(wavelength, β2_list, lambda_vec, imode=N_MODES):
    beta2interp = new_interpolation(lambda_vec[:-2], β2_list[imode-1]) 
    index=np.argmin(beta2interp(wavelength.to(u.m))**2)
...
def gain_density_analysis(wavelength, β2_list, λ0vec, imode=N_MODES, save_path='./'):
    ...
    ax0.set_ylabel("Pump Wavelength [μm]")
    ax0.set_xlabel("$β_2$ [$ps^2$/nm/km]")
    ax0.grid()
    ax0=ax[1]
...
def gain_density_3plots_analysis(wavelength, β2_list, λ0vec, imode=N_MODES, save_path='./'):
    beta2interp = new_interpolation(lambda_vec[:-2], β2_list[imode-1]) 
    GainMatrix=np.zeros([len(wavelength),len(λ0vec)])
    λMatrix=np.zeros([len(wavelength),len(λ0vec)])
    λ0Matrix=np.zeros([len(wavelength),len(λ0vec)])
    ...
    fig,ax = plt.subplots(1,3,sharey=True, gridspec_kw={'width_ratios': [0.5, 1, 1]})
    ax0=ax[0]
    ax0.plot(beta2interp(λ0vec.to(u.m))*u.ps**2/u.km,λ0vec.value)
    ax0.set_ylabel("Pump Wavelength [μm]")
    ax0.set_xlabel("$β_2$ [$ps^2$/nm/km]")
    ...
    ax0.set_xlabel("Wavelength [μm]")
    ax0=ax[2]
    c = ax0.pcolormesh(λMatrix,λ0Matrix, (GainMatrix), cmap='plasma', vmin=GainMatrix.min(), vmax=GainMatrix.max(), rasterized=True)
    plt.colorbar(c, label='Gain [dB]')
    ax0.grid()
    ax0.set_xlim([-0.4,0.4])
    ax0.set_xlabel("λ-λ$_p$ [μm]")
        
    save_file = os.path.join(save_path, 'parametric_gain_density_wavelength_3plot.png')
    plt.savefig(save_file, bbox_inches='tight', dpi=300)
    plt.clf()
    plt.close()
...
if __name__ == '__main__':
    ...
    for d_core in D_core_values:
        ...
        print(f"\n\nStarting analysis of D_core={d_core:.2e}")

        df = raw_df.loc[raw_df['D_core (m)'] == d_core]
        save_dir = os.path.join(SAVE_PATH, f'{d_core*1e6:.2f}um diameter')
        ...
        γ_calc = np.array(df["Nonlinear coefficient"]) /(u.m * u.W)
        if N_MODES > 1:
            γ = np.average(γ_calc)*np.ones_like(wavelength.value)
            print("Using average value for gamma: ", np.average(γ_calc))
        else:
            γ_interp = new_interpolation(wv0_values*u.m, γ_calc)
            γ = γ_interp(wavelength)/(u.m * u.W)
        ...
        print(f"Value of ZDW: {zero_GVD:.3e}")
        ...
        gain_density_3plots_analysis(wavelength, β2_list, λ0vec, imode=N_MODES, save_path=save_dir)